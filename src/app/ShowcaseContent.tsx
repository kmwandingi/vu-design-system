        ...
        <TabsContent value="foundations" className="space-y-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <section>
              ...
              <ShowcaseItem code={`// Brand color utility classes — use directly in className
...
<div className="rounded-lg p-6 bg-vu-blue text-white">
  ...
  <p className="text-sm opacity-90">#0077B3</p>
            ...
            <section>
              ...
              <ShowcaseItem code={`import { badgeVariants } from '@vu/design-system';
...
<span className={badgeVariants({ variant: 'outline' })}>Draft</span>`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <section>
              ...
              <div className="space-y-6">
                <div><h1>Heading 1 (2.25rem / 36px)</h1></div>
                <div><h2>Heading 2 (1.875rem / 30px)</h2></div>
                <div><h3>Heading 3 (1.5rem / 24px)</h3></div>
                <div><h4>Heading 4 (1.25rem / 20px)</h4></div>
                <div><p>Body text (1rem / 16px) — default text size for most content.</p></div>
                <div><small>Small text (0.875rem / 14px) — captions, labels, secondary info.</small></div>
              </div>
            </section>
            <section>
              <SectionHeader title="Shadows" />
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {['shadow-xs','shadow-sm','shadow-md','shadow-lg','shadow-xl','shadow-2xl','shadow-blue','shadow-orange'].map((shadow) => (
                  <ShadowCard key={shadow} name={shadow} className={shadow} />
                ))}
              </div>
            </section>
          </div>
          ...
          <section>
            ...
            <ShowcaseItem code={`import { Icons } from '@vu/design-system';
          ...
          <section>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-x-12 gap-y-8 mb-8">
              {[
                ...
                <ShowcaseItem key={title} code={`import { buttonVariants } from '@vu/design-system';
...
<button className={buttonVariants({ variant: '${d}' })}>Ghost</button>`}>
                  <div>
                    <h3 className={cn('text-lg font-semibold mb-4', title.includes('Blue') ? 'text-vu-blue-700' : title.includes('Purple') ? 'text-vu-purple-700' : title.includes('Orange') ? 'text-vu-orange-700' : 'text-vu-green-700')}>{title}</h3>
                    <div className="flex flex-wrap gap-3">
                      {[a,b,c,d].map((variant) => (
                        <button key={variant} className={buttonVariants({ variant: variant as never })}>
                          {String(variant).split('-')[1] ? String(variant).split('-')[1][0].toUpperCase()+String(variant).split('-')[1].slice(1) : String(title).split(' ')[0]}
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            <ShowcaseItem code={`import { buttonVariants } from '@vu/design-system';
...
<button className={buttonVariants({ variant: 'primary', size: 'xl' })}>XL</button>`}>
              <div className="mb-8">
                <h3 className="text-lg font-semibold mb-4 text-muted-foreground">Button Sizes</h3>
                <div className="flex flex-wrap items-center gap-3">
                  {['xs','sm','md','lg','xl'].map((size) => (
                    <button key={size} className={cn(buttonVariants({ variant: 'primary', size: size as never }), 'transition-colors duration-150')}>{size.toUpperCase()}</button>
                  ))}
                </div>
              </div>
            </ShowcaseItem>
...
<button className={buttonVariants({ variant: 'secondary' })}>
  ...
  Download
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  <button className={buttonVariants({ variant: 'primary' })}><Icons.Actions.Search className="h-4 w-4" />Search</button>
                  <button className={buttonVariants({ variant: 'secondary' })}><Icons.Actions.Download className="h-4 w-4" />Download</button>
                  <button className={buttonVariants({ variant: 'accent' })}><Icons.Status.Star className="h-4 w-4" />Favorite</button>
                  <button className={buttonVariants({ variant: 'tertiary' })}><Icons.Status.CheckCircle className="h-4 w-4" />Complete</button>
                </div>
              </div>
            </ShowcaseItem>
          </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <section>
              ...
              <ShowcaseItem code={`import { cardStyles } from '@vu/design-system';
...
<div className={cardStyles({ variant: 'interactive', tone: 'secondary', accent: 'left', padding: 'md' })}>
  Interactive card — hover effects plus semantic grouping
                ...
                <div className="space-y-4">
                  <div className={cardStyles({ variant: 'default', padding: 'md' })}><h3 className="font-semibold mb-2">Default Card</h3><p className="text-sm text-muted-foreground">A standard card with border and subtle shadow.</p></div>
                  <div className={cardStyles({ variant: 'default', tone: 'primary', accent: 'left', padding: 'md' })}><h3 className="font-semibold mb-2">Left-accent card</h3><p className="text-sm text-muted-foreground">Use this for featured decisions, highlighted evidence, and semantic grouping.</p></div>
                  <div className={cardStyles({ variant: 'elevated', tone: 'accent', accent: 'top', padding: 'md' })}><h3 className="font-semibold mb-2">Top-accent card</h3><p className="text-sm text-muted-foreground">Use this for metrics and compact summary cards that need scanable differentiation.</p></div>
                  <div className={cardStyles({ variant: 'interactive', tone: 'secondary', accent: 'left', padding: 'md' })}><h3 className="font-semibold mb-2">Interactive Card</h3><p className="text-sm text-muted-foreground">A clickable card with hover effects and restrained semantic emphasis.</p></div>
                </div>
              </ShowcaseItem>
            </section>
            ...
            <section>
              ...
              <ShowcaseItem code={`import { alertVariants } from '@vu/design-system';
...
<div className={cn(alertVariants({ variant: 'default' }), 'border-vu-blue/50 bg-vu-blue-50 text-vu-blue-800 [&>svg]:text-vu-blue-600')}>
  ...
  <div>
    ...
    <p className="text-sm">This is a primary alert using VU Blue.</p>
                ...
                <div className="space-y-4">
                  <AlertPreview title="Primary Alert"  text="This is a primary alert using VU Blue."   className="border-vu-blue/50 bg-vu-blue-50 text-vu-blue-800 [&>svg]:text-vu-blue-600"     icon={<Icons.Status.Info className="h-4 w-4" />} />
                  <AlertPreview title="Success Alert"  text="This is a success alert using VU Green."  className="border-[#008053]/30 bg-[#E6F5F0] text-[#004D32] [&>svg]:text-[#008053]"      icon={<Icons.Status.CheckCircle className="h-4 w-4" />} />
                  <AlertPreview title="Warning Alert"  text="This is a warning alert using warm amber." className="border-[#F59E0B]/30 bg-[#FFFBEB] text-[#92400E] [&>svg]:text-[#D97706]"     icon={<Icons.Status.Warning className="h-4 w-4" />} />
                  <AlertPreview title="Error Alert"    text="This is an error alert using red."        className="border-[#DC2626]/30 bg-[#FEF2F2] text-[#7F1D1D] [&>svg]:text-[#B91C1C]"      icon={<Icons.Status.ErrorCircle className="h-4 w-4" />} />
                </div>
              </ShowcaseItem>
            </section>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <section>
              ...
              <ShowcaseItem code={`import { FormField, InfoHint } from '@vu/design-system';
...
<FormField label="Email" error="Please enter a valid email.">
  <input type="email" className="w-full rounded-lg border-2 border-error-500/50 px-4 py-2.5" />
                ...
                <div className="space-y-8">
                  <FormField label="Textfield/Default"><input type="text" placeholder="Textfield text" className="w-full px-4 py-2.5 rounded-lg border border-vu-blue-200 bg-vu-blue-50/20 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-vu-blue/30 focus:border-vu-blue" /></FormField>
                  <FormField label="Textfield/Valid"><div className="relative"><input type="email" defaultValue="email@domain.com" className="w-full px-4 py-2.5 pr-10 rounded-lg border-2 border-vu-green/50 bg-vu-green-50/30 text-vu-green-900 focus:outline-none" /><Icons.Status.CheckCircle className="absolute right-3 top-1/2 -translate-y-1/2 h-5 w-5 text-vu-green" /></div></FormField>
                  <FormField label="Textfield/Error" error="Please enter a valid email address"><div className="relative"><input type="email" defaultValue="email@" className="w-full px-4 py-2.5 pr-10 rounded-lg border-2 border-error-500/50 bg-error-50/30 text-error-900 focus:outline-none" /><Icons.Status.ErrorCircle className="absolute right-3 top-1/2 -translate-y-1/2 h-5 w-5 text-error-500" /></div></FormField>
              ...
              <div className="mt-6">
                <ShowcaseItem code={`import { FormField, InfoHint, SectionHeader, buttonVariants } from '@vu/design-system';
  ...
  {[['Loose', 30], ['Balanced', 65], ['Strict', 85]].map(([label, val]) => (
    <button key={label} onClick={() => setThreshold(val)}
      ...
      {label}
                  ...
                  <div className={cardStyles({ variant: 'default', padding: 'md' })}>
                    ...
                    <div className="mt-4 space-y-4">
                      ...
                      >
                        <div className="space-y-2">
                          <div className="flex items-center gap-3">
                            <input type="range" min="0" max="100" value={threshold} onChange={e => setThreshold(Number(e.target.value))} className="flex-1 accent-vu-blue" />
                            <span className={cn('w-14 text-center text-sm font-semibold rounded-full px-2 py-0.5 transition-colors duration-300', threshold >= 75 ? 'bg-vu-orange-50 text-vu-orange' : threshold <= 40 ? 'bg-vu-green-50 text-vu-green' : 'bg-vu-blue-50 text-vu-blue')}>{threshold}%</span>
                          </div>
                          <div className="flex justify-between text-xs text-muted-foreground px-0.5">
                            <span>Loose</span>
                            <span className={cn('transition-colors duration-300', threshold >= 75 ? 'text-vu-orange font-medium' : threshold <= 40 ? 'text-vu-green font-medium' : 'text-vu-blue font-medium')}>{threshold <= 40 ? 'More results' : threshold >= 75 ? 'Fewer, higher confidence' : 'Balanced'}</span>
                            <span>Strict</span>
                          </div>
                        </div>
                      </FormField>
                      <div className="flex gap-2 pt-1">
                        {([['Loose', 30], ['Balanced', 65], ['Strict', 85]] as const).map(([label, val]) => (
                          <button key={label} onClick={() => setThreshold(val)} className={cn(buttonVariants({ variant: threshold === val ? 'primary' : 'outline', size: 'sm' }), 'flex-1 transition-colors duration-150')}>{label}</button>
                        ))}
                      </div>
                    </div>
                  </div>
            ...
            <section>
              <SectionHeader title="Feedback States" description="LoadingState for multi-step page loads. Spinner for inline async actions. Skeleton for initial content load. For agent-driven async, see AIState in the AI & Async tab." />
              <div className="space-y-6">
                <ShowcaseItem code={`import { LoadingState } from '@vu/design-system';
...
<LoadingState
  ...
  progress={68}
/>`}>
                  <LoadingState title="Building your page" description="Tokens, layouts, and content are being assembled into a polished interface." progress={loadingProgress} />
                </ShowcaseItem>

                <ShowcaseItem code={`import { Spinner, Progress } from '@vu/design-system';
...
// sizes: sm · md · lg
<Progress value={72} variant="primary" size="lg" />`}>
                  <div className={cardStyles({ variant: 'default', padding: 'md' })}>
                    <div className="flex items-center justify-between gap-4 mb-4">
                      <div><h3 className="font-semibold">Inline async action</h3><p className="text-sm text-muted-foreground">Use Spinner for button-level or field-level async — not for full-page loads.</p></div>
                      <Spinner size="md" variant="primary" label="Syncing" />
                    </div>
                    <Progress value={statusProgress} variant="primary" size="lg" />
                  </div>
                </ShowcaseItem>

                <ShowcaseItem code={`import { Skeleton } from '@vu/design-system';
...
<Skeleton variant="button" />`}>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className={cardStyles({ variant: 'default', padding: 'md' })}><Skeleton variant="text" className="mb-3" /><Skeleton variant="text" className="w-4/5 mb-2" /><Skeleton variant="text" className="w-3/5" /></div>
                    <div className={cardStyles({ variant: 'default', padding: 'md' })}><Skeleton variant="avatar" className="mb-3" /><Skeleton variant="text" className="w-2/3" /></div>
                    <div className={cardStyles({ variant: 'default', padding: 'md' })}><Skeleton variant="button" className="mb-3" /><Skeleton variant="text" className="w-1/2" /></div>
                  </div>
                </ShowcaseItem>
              </div>
            </section>
        ...
        <TabsContent value="workflow" className="space-y-12">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-12 items-start">
            <div className="space-y-6">
              <ShowcaseItem code={`import { Tabs, TabsList, TabsTrigger, TabsContent } from '@vu/design-system';
...
<Tabs defaultValue="overview">
  ...
  <TabsContent value="governance">
    Governance content
                ...
                <div className={cardStyles({ variant: 'default', padding: 'lg' })}>
                  <SectionHeader title="Tabs" description="Use tabs for short, closely related views inside a shared context." />
                  <Tabs defaultValue="overview" className="mt-4">
                    <TabsList><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="activity">Activity</TabsTrigger><TabsTrigger value="governance">Governance</TabsTrigger></TabsList>
                    <TabsContent value="overview" className="rounded-lg border bg-card p-4 text-sm text-muted-foreground">Shared foundations keep the system visually coherent across pages, teams, and agent-generated interfaces.</TabsContent>
                    <TabsContent value="activity"   className="rounded-lg border bg-card p-4 text-sm text-muted-foreground">Activity and workflow surfaces should be lightweight and easy to scan.</TabsContent>
                    <TabsContent value="governance" className="rounded-lg border bg-card p-4 text-sm text-muted-foreground">Package-first adoption helps teams enforce the design language with less drift.</TabsContent>
                  </Tabs>
                </div>
              </ShowcaseItem>

              <ShowcaseItem code={`import { FormField } from '@vu/design-system';
  ...
  <input type="email" defaultValue="kenneth@"
    className="w-full rounded-lg border-2 border-error-500/40 bg-error-50/40 px-4 py-2.5 text-sm
               focus:outline-none" />
                ...
                <div className={cardStyles({ variant: 'default', padding: 'lg' })}>
                  <SectionHeader title="Form composition" description="FormField creates a reusable label, helper, and error pattern." />
                  <div className="mt-4 grid gap-5">
                    <FormField label="Project title" description="A concise descriptive name for the initiative." required><input type="text" placeholder="Award-winning VU experience" className="w-full rounded-lg border bg-muted/50 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" /></FormField>
                    <FormField label="Owner email" error="Please provide a valid institutional email address."><input type="email" defaultValue="kenneth@" className="w-full rounded-lg border-2 border-error-500/40 bg-error-50/40 px-4 py-2.5 text-sm focus:outline-none" /></FormField>
                  </div>
                </div>
              </ShowcaseItem>
            </div>
            ...
            <div className="space-y-6">
              <ShowcaseItem code={`import { Stepper, ProgressDots } from '@vu/design-system';
...
<ProgressDots total={5} current={3} />`}>
                <div className={cardStyles({ variant: 'default', padding: 'lg' })}>
                  <SectionHeader title="Stepper" description="For multi-step creation, onboarding, and guided decision flows." />
                  <Stepper className="mt-4" items={[{ title: 'Intent', description: 'Clarify the problem and outcome.', status: 'complete' }, { title: 'Structure', description: 'Compose the interface from reusable system parts.', status: 'current' }, { title: 'Ship', description: 'Package, test, and distribute across projects.', status: 'upcoming' }]} />
                  <div className="mt-4 flex items-center justify-between rounded-lg border bg-muted/30 px-4 py-3">
                    <div><p className="text-sm font-medium">Compact flow indicator</p><p className="text-xs text-muted-foreground">Useful for onboarding, task flows, and guided forms.</p></div>
                    <ProgressDots total={5} current={3} />
                  </div>
                </div>
              </ShowcaseItem>

              <ShowcaseItem code={`import { Timeline } from '@vu/design-system';
...
<Timeline items={[
  ...
  { title: 'Adoption',     description: 'Package for team consumption.',             meta: 'Next',        status: 'upcoming' },
                ...
                <div className={cardStyles({ variant: 'default', padding: 'lg' })}>
                  <SectionHeader title="Journey timeline" description="Use timelines when users need narrative progression instead of discrete step navigation." />
                  <Timeline className="mt-4" items={[{ title: 'Discovery', description: 'Clarified the system language and the component responsibilities.', meta: 'Done', status: 'complete' }, { title: 'Composition', description: 'Assembled reusable primitives for loading, progress, and structured status flows.', meta: 'In progress', status: 'current' }, { title: 'Adoption', description: 'Package the system so teams and agents can consume it consistently across products.', meta: 'Next', status: 'upcoming' }]} />
                </div>
              </ShowcaseItem>

              <ShowcaseItem code={`import { EmptyState, buttonVariants } from '@vu/design-system';
...
/>`}>
                <EmptyState title="No filtered results" description="When users narrow a list down to nothing, give them a calm explanation and an obvious next action." actions={<button className={buttonVariants({ variant: 'primary' })}>Reset filters</button>} />
              </ShowcaseItem>
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="data" className="space-y-12">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-12 items-start">
            <div className="space-y-6">
              <ShowcaseItem code={`import { StatCard } from '@vu/design-system';
  ...
  icon={<Star className="h-4 w-4" />} />`}>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <StatCard label="Completion rate" value="82%" trend="+6.4% vs last month" meta="Across active journeys" icon={<Icons.Analytics.TrendingUp className="h-4 w-4 text-vu-green" />} />
                    <StatCard label="Open opportunities" value="124" trend="12 newly matched" meta="Funding discovery surface" icon={<Icons.Documents.Folder className="h-4 w-4 text-vu-orange" />} />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <StatCard label="Clusters" value="12" size="compact" icon={<Icons.Analytics.Workflow className="h-4 w-4" />} />
                    <StatCard label="Flagged passages" value="48" size="compact" hint="Highlights combine extracted evidence from the primary and compared document review panes." icon={<Icons.Status.Star className="h-4 w-4" />} />
                    <StatCard label="Review confidence" value="High" size="compact" meta="Updated after comparison analysis" icon={<Icons.Status.CheckCircle className="h-4 w-4 text-vu-green" />} />
                  </div>
                </div>
              </ShowcaseItem>

              <ShowcaseItem code={`import { StatusPanel } from '@vu/design-system';
...
/>`}>
                <StatusPanel title="Application shortlist in progress" description="Candidate programmes are being compared against user preferences and completion evidence." progress={statusProgress} variant="info" icon={<Icons.Status.Info className="h-4 w-4 text-vu-blue" />} meta="Updated 40 seconds ago" />
              </ShowcaseItem>

              <ShowcaseItem code={`import { KeyValueList } from '@vu/design-system';
...
<KeyValueList size="compact" keyWidthClassName="minmax(0,120px)" items={[
  ...
  { key: 'Matched on',   value: 'Title, content, metadata' },
                ...
                <div className={cardStyles({ variant: 'default', padding: 'lg' })}>
                  <SectionHeader title="Structured data" description="Tables and key-value lists should feel readable rather than heavy." />
                  <div className="mt-4 space-y-6">
                    <KeyValueList items={[{ key: 'Programme', value: 'Business Analytics' }, { key: 'Faculty', value: 'School of Business and Economics' }, { key: 'Language', value: 'English' }]} />
                    <KeyValueList size="compact" keyWidthClassName="minmax(0,120px)" items={[{ key: 'Primary file', value: 'Advice memo v4.docx' }, { key: 'Cluster', value: '12' }, { key: 'Matched on', value: 'Title, content, metadata' }]} />
                  </div>
                </div>
              </ShowcaseItem>

              <ShowcaseItem code={`import { Table, TableHeader, TableBody, TableRow, TableHead, TableCell, TableCaption } from '@vu/design-system';
...
<Table>
  ...
  <TableCaption>Example data table</TableCaption>
                ...
                <Table>
                  <TableHeader><TableRow><TableHead>Project</TableHead><TableHead>Status</TableHead><TableHead>Owner</TableHead></TableRow></TableHeader>
                  <TableBody>
                    <TableRow><TableCell>Funding Explorer</TableCell><TableCell><span className="inline-flex items-center rounded-full bg-vu-orange-50 px-2.5 py-0.5 text-xs font-medium text-vu-orange-800">In review</span></TableCell><TableCell>Kenneth</TableCell></TableRow>
                    <TableRow><TableCell>Matching Journey</TableCell><TableCell><span className="inline-flex items-center rounded-full bg-vu-green-50 px-2.5 py-0.5 text-xs font-medium text-vu-green-800">Live</span></TableCell><TableCell>VU Team</TableCell></TableRow>
                  </TableBody>
                  <TableCaption>Example data-display shell for shared admin or decision interfaces.</TableCaption>
                </Table>
              </ShowcaseItem>
            </div>

            <div className="space-y-6">
              <ShowcaseItem code={`import { FilterBar, SearchInput, badgeVariants, buttonVariants } from '@vu/design-system';
...
/>`}>
                <div className="space-y-4">
                  <ResultSummary title="Recommended next step" summary="The current evidence suggests surfacing a small shortlist with strong programme-fit rationale and a clear next action." highlights="Shortlist business, data, and behavioural science pathways. Show one confident recommendation, then present two alternative paths with evidence." actions={<button className={buttonVariants({ variant: 'secondary-outline' })}>View shortlist</button>} />
                  <ResultSummary eyebrow="Review summary" title="Duplicate-review decision" summary="The evidence indicates this pair is safe to collapse into a single retained document, with a manual check on naming before tagging." highlights={<div className="space-y-2"><p>Strong content overlap with minor filename drift.</p><p>Metadata and extraction timestamps support consolidation.</p></div>} meta="Confidence high · 2 documents · Last analysed 40 seconds ago" tone="info" actions={<><button className={buttonVariants({ variant: 'primary' })}>Accept recommendation</button><button className={buttonVariants({ variant: 'outline' })}>Inspect evidence</button></>} />
                </div>
              </ShowcaseItem>

              <ShowcaseItem code={`import { Pagination, PaginationContent, PaginationItem, PaginationLink,
...
<Pagination>
  <PaginationContent>
    ...
    <PaginationItem><PaginationNext href="#" /></PaginationItem>
                ...
                <div className={cardStyles({ variant: 'default', padding: 'lg' })}>
                  <SectionHeader title="Pagination" description="For dense listings and structured navigation across larger datasets." />
        ...
        <TabsContent value="ai" className="space-y-12">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-12 items-start">
            <div className="space-y-6">
              <ShowcaseItem code={`import { AIState } from '@vu/design-system';
...
<AIState
  ...
  progress={74}
/>`}>
                <AIState title="Synthesising guidance" description="The assistant is clustering relevant evidence, ranking options, and composing a concise recommendation." status={`Reasoning · ${aiProgress}% complete`} progress={aiProgress} />
              </ShowcaseItem>

              <ShowcaseItem code={`import { ActivityFeedItem } from '@vu/design-system';
...
<ActivityFeedItem
  ...
  meta="2 min ago"
/>`}>
                <ActivityFeedItem icon={<Icons.Analytics.Activity className="h-4 w-4 text-vu-purple" />} title="Model summary refreshed" description="Re-ranked evidence after new programme metadata was added to the comparison set." meta="2 min ago" />
              </ShowcaseItem>

              <ShowcaseItem code={`import { EvidenceCard } from '@vu/design-system';
...
/>`}>
                <EvidenceCard title="Primary source evidence" source="Student survey synthesis · Confidence high" summary="Students who prefer structured, analytical work and interdisciplinary collaboration show the highest completion and satisfaction in this route." relevance="Most relevant to current recommendation" />
              </ShowcaseItem>

              <ShowcaseItem code={`import { DataVisualShell } from '@vu/design-system';
...
>
  ...
  <div className="grid grid-cols-6 gap-2">
    ...
    {/* ... */}
                ...
                <DataVisualShell title="Advanced data visual shell" subtitle="A lightweight chart container and framing component for richer dashboard modules." footer="Use this shell to standardise spacing, actions, annotations, and explanatory text around data visualisations.">
                  <div className="grid grid-cols-6 gap-2"><div className="h-20 rounded bg-vu-blue-200" /><div className="h-28 rounded bg-vu-purple-300" /><div className="h-16 rounded bg-vu-green-200" /><div className="h-32 rounded bg-vu-blue-400" /><div className="h-24 rounded bg-vu-orange-300" /><div className="h-36 rounded bg-vu-green-500" /></div>
                </DataVisualShell>
              </ShowcaseItem>
            </div>

            <div className="space-y-6">
              <ShowcaseItem code={`import { WorkflowAuditTimeline } from '@vu/design-system';
...
/>`}>
                <div className={cardStyles({ variant: 'default', padding: 'lg' })}>
                  <SectionHeader title="Workflow audit trail" description="A specialised timeline for AI-assisted, reviewable system actions." />
                  <WorkflowAuditTimeline className="mt-4" items={[{ title: 'Sources retrieved', description: 'Pulled fit signals from user profile, programme metadata, and prior responses.', meta: '12:01', status: 'complete' }, { title: 'Evidence ranked', description: 'Scored and clustered the most relevant decision factors.', meta: '12:02', status: 'complete' }, { title: 'Recommendation drafted', description: 'Composing a recommendation with explanation and next-step actions.', meta: '12:03', status: 'current' }]} />
                </div>
              </ShowcaseItem>

              <ShowcaseItem code={`import { InlineEditable } from '@vu/design-system';

// Click the text to edit inline — no modal required
<InlineEditable value="Student-fit recommendation title" />`}>
                <div className={cardStyles({ variant: 'default', padding: 'lg' })}>
                  <SectionHeader title="Inline editing" description="Useful for profile surfaces, settings, or structured data refinement." />
                  <InlineEditable className="mt-4" value="Student-fit recommendation title" />
                </div>
              </ShowcaseItem>
            </div>
          </div>
...
// ---------------------------------------------------------------------------
function ShowcaseItem({ code, children }: { code: string; children: ReactNode }) {
  ...
  return (
    <div className="group/si relative">
      <div className="relative">
        {children}
        <button
          onClick={() => setOpen(o => !o)}
          ...
          className={cn(
            'absolute top-2 right-2 flex items-center gap-1 rounded-md border px-2 py-1 text-xs transition-colors duration-150',
            'opacity-0 group-hover/si:opacity-100',
            open
              ? 'bg-vu-blue text-white border-vu-blue'
              : 'bg-background/90 text-muted-foreground border-border hover:border-vu-blue/40 hover:text-vu-blue',
          )}
        >
      ...
      {open && (
        <div className="mt-2 rounded-lg border overflow-hidden shadow-sm">
          <div className="flex items-center justify-between px-4 py-2 bg-slate-800 border-b border-slate-700">
            <span className="text-xs font-mono text-slate-400">JSX — copy and paste into your project</span>
            <button
              onClick={copy}
              className="flex items-center gap-1.5 text-xs text-slate-300 hover:text-white transition-colors duration-150"
            >
              {copied
                ? <><Check className="h-3 w-3 text-vu-green" /> Copied!</>
                : <><Copy className="h-3 w-3" /> Copy</>}
            </button>
          </div>
          <pre className="overflow-x-auto bg-slate-900 p-4 text-xs font-mono text-slate-200 leading-relaxed whitespace-pre">
            <code>{code.trim()}</code>
          </pre>
        </div>
      )}
...
function IconItem({ icon, name }: { icon: ReactNode; name: string }) {
  return <div className="flex w-[88px] shrink-0 flex-col items-center gap-2 rounded-lg p-3 text-center hover:bg-muted transition-colors duration-150">{icon}<span className="text-xs text-muted-foreground">{name}</span></div>;
}
